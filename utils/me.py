...
if __name__ == '__main__':
    ...
    if configparse('common').getboolean('speedtest_enabled'):
        ...
        os.system(f'python3 ./utils/litespeedtest/speedtest.py --subscription \"../../{subscription}\" --range \"200,1100\" --path \"../../temp\"')
        ...
        other_country_proxies = convert('../../temp','base64',{'deduplicate':False,'rename':'','include':'','exclude':'','config':''})

        area_proxies = {
            #'east_asia': [east_asian_proxies, 20],
            #'north_america': [north_america_proxies, 20],
        ...
        for area in area_proxies.keys():
            ...
            os.system(f'python3 ./utils/litespeedtest/speedtest.py --subscription \"../../temp\" --range \"{area_proxies[area][1]}\" --path \"../../temp\"')
            with open('./temp', 'r', encoding='utf-8') as temp_file:
                content = temp_file.read()
                share_proxies.append(base64_decode(content))
        with open('./Eternity', 'w', encoding='utf-8') as temp_file:
